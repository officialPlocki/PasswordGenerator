...
public class FilenameUtils {
  ...
  private static String doNormalize(String filename, boolean keepSeparator) {
    ...
    for (j = prefix + 1; j < size; j++) {
      if (array[j] == SYSTEM_SEPARATOR && array[j - 1] == '.' && (j == prefix + 1 || array[j - 2] == SYSTEM_SEPARATOR)) {
        if (j == size - 1)
    ...
    for (j = prefix + 2; j < size; j++) {
      if (array[j] == SYSTEM_SEPARATOR && array[j - 1] == '.' && array[j - 2] == '.' && (j == prefix + 2 || array[j - 3] == SYSTEM_SEPARATOR)) {
        if (j == prefix + 2)
          return null; 
        if (j == size - 1)
          lastIsDirectory = true; 
  ...
  static String[] splitOnTokens(String text) {
    ...
    if (buffer.length() != 0)
      list.add(buffer.toString()); 
    return (String[]) list.<String>toArray(new String[list.size()]);
  }
}
